...
export default function CartModal({setShow}: Props) {
  return (
    <div className="fixed top-0 left-0 w-screen h-screen flex justify-center items-center bg-gray-600 bg-opacity-50 z-10" onClick={() => setShow(false)}>
      <div className="w-96 h-auto rounded-lg bg-white z-20 flex flex-col gap-4 items-center"
        ...
        <button className="w-1/2 text-white text-xl font-bold bg-yellow-600 p-3 mb-3 rounded-md"
          onClick={() => setShow(false)}>Замовити</button>
      </div>      
    </div>
  )
}